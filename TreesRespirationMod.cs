...
namespace Klyte.TreesRespiration
{
    public class TreesRespirationMod : BasicIUserMod<TreesRespirationMod, TRPResourceLoader, MonoBehaviour, TRPCommonTextureAtlas, UICustomControl, SpriteNames>
    {
        ...
        {
            m_redirector = KlyteMonoUtils.CreateElement<Redirector>(null, "TRP");
        }

        public override void TopSettingsUI(UIHelperExtension ext)
        {
            var group = ext.AddGroupExtended(Locale.Get("K45_TREE_STRENGTH_MULTIPLIER"));
        ...
        {
            if (MultiplierTrees.value == 0) return;
            var frameIdx = SimulationManager.instance.m_currentFrameIndex;
        ...
        {
            if (MultiplierBuildings.value == 0 || data.Info == null) return;
        ...
        {
            if (MultiplierNet.value == 0 || data.Info == null) return;
            for (int l = 0; l < data.Info.m_lanes.Length; l++)
            {
                if (data.Info.m_lanes[l].m_laneProps == null) continue;
